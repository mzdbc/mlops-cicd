...
```bash
conda create -n mlops_cicd python=3.9
conda activate mlops_cicd
```
...
```
This command will create your personal repository under `/Repos/<username>/mlops_cicd`.
...
- To trigger the release pipeline, get the current version from the `mlops_cicd/__init__.py` file and tag the current code version:
```
